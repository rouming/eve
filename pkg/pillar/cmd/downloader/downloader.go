...
func lookupDatastore(dsid_ uuid.UUID, status types.DownloaderStatus) bool {
	for _, dsid := range status.DatastoreIDsList {
		if dsid == dsid_ {
			return true
		}
	}
...
func handleCreate(ctx *downloaderContext, config types.DownloaderConfig,
	...
	if status == nil {
		...
		status0 := types.DownloaderStatus{
			...
			DatastoreIDsList: config.DatastoreIDsList,
			Name:             config.Name,
...
func doDownload(ctx *downloaderContext, config types.DownloaderConfig, status *types.DownloaderStatus,
	...
	dslist, err := prepareDatastoresList(ctx, config, config.DatastoreIDsList)
	if err != nil {
	...
	for i, ds := range dslist {
		...
		if errStr != "" {
			...
			// or download has been cancelled
			if cancelled || (i+1) == len(dslist) {
				if !cancelled {
